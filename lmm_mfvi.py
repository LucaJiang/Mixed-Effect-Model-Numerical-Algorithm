...
def lmm_em_mfvi(y, X, Z, tol=1e-6, max_iter=10, verbose=True):
    '''
    Input:
    y: n x 1, response
    ...
    mu = Gamma * ( X^T * (y - Z * omega) / sigma_e^2 + beta / sigma_beta^2)
    beta = mu
    # e = y - Z * omega - X * beta
...
if __name__ == '__main__':
    ...
    # run EM algorithm
    likelihood_list, omega_list, sigma_beta2_list, sigma_e2_list, beta_post_mean = lmm_em_mfvi(
        y, X, Z)
    ...
    plt.tight_layout()
    plt.savefig('img/lmm_em_mfvi' + data_name + '.png')
    plt.show()
